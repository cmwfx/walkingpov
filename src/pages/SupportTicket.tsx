...
export function SupportTicket() {
  ...
  return <div className="container mx-auto max-w-3xl px-4 py-10"><Link to={isAdmin ? '/admin/support' : '/support'}><Button variant="ghost" className="text-slate-300"><ArrowLeft className="mr-2 h-4 w-4" />All tickets</Button></Link><Card className="mt-5 border-white/10 bg-white/[0.05] text-white"><CardHeader><div className="flex items-start justify-between gap-4"><div><CardTitle>{ticket.subject}</CardTitle><p className="mt-2 text-sm text-slate-400">{ticket.status === 'open' ? 'Open' : 'Closed'} · Started {new Date(ticket.created_at).toLocaleDateString()}</p></div>{(isAdmin || ticket.status === 'closed') && <Button variant="outline" size="sm" className="border-white/15 text-white" onClick={() => void toggle()}>{ticket.status === 'closed' ? <><RotateCcw className="mr-2 h-4 w-4" />Reopen</> : <><CheckCircle className="mr-2 h-4 w-4" />Close</>}</Button>}</div></CardHeader><CardContent><div className="space-y-3">{ticket.messages.map((message) => <div className={`rounded-xl border p-4 ${message.author_id === user?.id ? 'border-violet-300/20 bg-violet-300/10' : 'border-white/10 bg-black/20'}`} key={message.id}><p className="whitespace-pre-wrap text-sm leading-6">{message.body}</p><p className="mt-3 text-xs text-slate-500">{message.author_id === user?.id ? 'You' : 'CandidFan support'} · {new Date(message.created_at).toLocaleString()}</p></div>)}</div>{canReply && <form onSubmit={(event) => void submit(event)} className="mt-6 space-y-3"><Textarea value={body} maxLength={10000} required onChange={(e) => setBody(e.target.value)} placeholder="Write a reply…" />{isAdmin && <label className="flex items-center gap-2 text-sm text-slate-300"><input type="checkbox" checked={notifyEmail} onChange={(e) => setNotifyEmail(e.target.checked)} />Send a generic email notice</label>}{error && <p className="text-sm text-rose-300">{error}</p>}<Button className="bg-gradient-to-r from-violet-600 to-sky-600">Send reply</Button></form>}</CardContent></Card></div>;
}
